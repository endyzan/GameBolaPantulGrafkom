...
def create_sparkles(position, color):
    sparkles = []
    for _ in range(5):  # Mengurangi jumlah percikan untuk efek yang lebih halus
        sparkle = sphere(pos=position, radius=0.1, color=color, emissive=True)
        sparkle.velocity = random_velocity() * 2
        sparkles.append(sparkle)
    return sparkles

# Fungsi untuk membuat garis pigora bertitik
def create_frame_lines():
    lines = []
    segment_length = 1
    num_segments = 5  # Menyesuaikan jumlah segmen agar sesuai dengan gambar yang diunggah
    
    for i in range(num_segments):
        lines.append(box(pos=vector(-5 + i * (segment_length + 1), 5, 0), size=vector(segment_length, 0.2, 0.1), color=color.white, emissive=True))
        lines.append(box(pos=vector(-5 + i * (segment_length + 1), -5, 0), size=vector(segment_length, 0.2, 0.1), color=color.white, emissive=True))
        lines.append(box(pos=vector(5, -5 + i * (segment_length + 1), 0), size=vector(0.2, segment_length, 0.1), color=color.white, emissive=True))
        lines.append(box(pos=vector(-5, -5 + i * (segment_length + 1), 0), size=vector(0.2, segment_length, 0.1), color=color.white, emissive=True))
    
    return lines

# Buat garis pigora bertitik
